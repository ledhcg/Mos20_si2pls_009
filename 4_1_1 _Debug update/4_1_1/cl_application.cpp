			...
			if (this->get_object_name() == nameParent) {
				...
				for (size_t i = 0; i < children.size(); i++) {
					if (children.at(i)->get_object_name() == nameParent) {
						...
						while (this->it_child != this->children.end()) {
						add_new_child((*(this->it_child)), nameParent, nameChild);
						cout << "successX" << endl;
						this->it_child++;
					}
...
void cl_application::add_new_child(cl_base* ob_parent, string nameParent, string nameChild) {
	...
	//	if (ob_parent->children.at(i)->get_object_name() == nameParent) {
	//		cl_2* ob_2;
	...
	while (ob_parent->it_child != ob_parent->children.end()) {
		if (get_object_name((*(ob_parent->it_child))) == nameParent) {
			cl_2* ob_2;
			ob_2 = new cl_2(*(ob_parent->it_child));
			ob_2->set_object_name(nameChild);
