...
void cl_application::processInput() {
	while (true) {
		char check;
		check = getchar();
		...
		if (check == '/') {
			scanElementsX(check, this);
		} else if (check == 'e') {
			if(checkFinish(check)) break;
		}
		else
			break;
	}
...
bool cl_application::checkFinish(char check) {
	if (check != '/') {
		string text;
		...
		char charNext2, charNext3;
		charNext2 = getchar();
		charNext3 = getchar();
		if (charNext2 == '/' && charNext3 == '\n') {
			return true;
		}
		else
			return false; /// ????
